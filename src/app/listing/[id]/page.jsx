    ...
    <div className="grid gap-6 lg:grid-cols-3">
      <div className="space-y-4 lg:col-span-2">
        ...
        <div className="flex snap-x gap-2 overflow-x-auto">
          {l.images.map((i) => <img key={i.id} src={i.url} alt={l.title} className={`h-72 snap-center rounded-xl object-cover ${l.status === "SOLD" ? "opacity-60" : ""}`} />)}
        </div>
        <h1 className={`text-2xl font-bold ${l.status === "SOLD" ? "line-through opacity-60" : ""}`}>{l.title}</h1>
        ...
        {l.status !== "SOLD" && (
          <div className="card">
            <div className="mb-2 flex items-center justify-between">
              <h2 className="flex items-center gap-2 font-semibold">Чат с <Avatar user={l.user} size={24} />{l.user.username}</h2>
              {me && me.id !== l.userId && <Link href={`/chat?l=${l.id}&u=${l.userId}`} className="text-xs underline">На весь экран</Link>}
            </div>
