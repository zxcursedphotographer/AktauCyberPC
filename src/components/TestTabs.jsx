      ...
      <div role="tablist" className="mb-4 flex flex-wrap gap-2">
        {types.map((t) => (
          <button
            ...
            aria-selected={tab === t}
            onClick={() => setTab(t)}
            className={`rounded-lg px-3 py-1.5 text-sm ${tab === t ? "bg-accent text-white" : "bg-slate-200 dark:bg-white/10"}`}
          >
            {LABEL[t]}
          </button>
        ))}
            ...
            <div className="mt-3 flex flex-wrap gap-2">
              {t.mediaUrls.map((u, i) => (
                <button
                  ...
                  className="group relative overflow-hidden rounded-lg border border-slate-200 dark:border-white/10 transition hover:border-accent"
                >
                  <img
                    src={u}
                    alt="Скриншот теста"
